namespace MultiThreading.Task3.MatrixMultiplier.Tests
{
    ...
    public class Tests
    {
        ...
        public void ParallelEfficiencyTest()
        {
            ...
            var parallelMatrixMultiplier = new MatricesMultiplierParallel();
            var stopwatchSync = new Stopwatch();
            stopwatchSync.Start();
            var resultSync = matrixMultiplier.Multiply(m1, m2);
            stopwatchSync.Stop();

            var stopwatchParallel = new Stopwatch();
            stopwatchParallel.Start();
            var resultParallel = parallelMatrixMultiplier.Multiply(m1, m2);
            stopwatchParallel.Stop();

            Assert.Less(stopwatchParallel.Elapsed, stopwatchSync.Elapsed);
        }
